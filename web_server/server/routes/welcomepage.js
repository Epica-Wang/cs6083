...
router.get('/home/:username', function(req, res, next){
  ...
  needs to be updated
  */
  const userHomePageQuery = 'select aid, aname from LikeArtist natural join Artist natural join User where userName = ? ; \
  select followingUserName from FollowUser where userName = ? ;\
  select pid, pTitle from User natural join Playlist where userName = ?';
